...
int main(int argc,char **argv) {
    ...
    const char *server=arg_value(argc,argv,"--server",getenv("P2P_VENDOR_SERVER") ? getenv("P2P_VENDOR_SERVER") : "www.easy4ipcloud.com");
...
#define LOAD(name) name=(void*)GetProcAddress(dll,#name)
    ...
    while(!stop_requested) {
        Sleep(5000);
        for(int i=0;i<map_count;i++) {
            if(!maps[i].connected) continue;
            int st=P2P_QueryChannelStatus(type,map_handles[i],maps[i].local_port);
